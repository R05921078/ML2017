...
#pd.DataFrame(data).to_csv('result/test_data.csv', encoding='big5', index=False, header=False);
t1 = time.time()
# setting parameters
...
pd.DataFrame(result).to_csv('result/ig_res_'+time.strftime("%m-%d %H_%M", time.localtime())+'.csv', encoding='big5', index=False, header=False);
#print(time.time()-t1)