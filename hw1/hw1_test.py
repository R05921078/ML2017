...
for testHour in range(4,8,1):
    for testTimes in range(7,8,2):
        learnRate = 0.5
        times = testTimes*1
        turn = 0
        hours = testHour
        w = np.zeros((18*hours+1, 1))
 
        prev_gra = np.zeros((18*hours+1, 1))
...
# evaluation ----------------------------------------------------------------
        ...
        result.append([hours, times,loss[0][0],w])
        #print(loss[0][0])

    pd.DataFrame(result).to_csv('result/record_'+time.strftime("%m-%d %H_%M", time.localtime())+'.csv', encoding='big5', index=False, header=False);
# evaluation end-------------------------------------------------------------    
#print(time.time()-t1)
...
for row in range(240):
    result.append( ['id_'+str(row), test_y[row][0]*(rowMax[9] - rowMin[9])+mean[9] ] )
